...
module IdpCaptcha
  # Image printer copied from RuCaptcha
  class Image
    class << self
      def random_color
        if IdpCaptcha.config.style == :colorful
          color = [random_color_seed, random_color_seed, random_color_seed]
          color[rand(3)] = 0.to_s(8)
          color
        elsif IdpCaptcha.config.style == :gray
          [18.to_s(8), 18.to_s(8), 28.to_s(8)]
        else
      ...
      def create(code)
        chars        = code.split('')
        all_left     = 20
        font_size    = IdpCaptcha.config.font_size
        ...
        command = <<-CODE
          ...
          -fill none \
          -implode #{IdpCaptcha.config.implode} -trim label:- png:-
        CODE
        ...
        if Gem.win_platform?
          ...
          warn "  IdpCaptcha #{err.strip}" if err.present?
          png_file_path
        else
          command.strip!
          out, err, _st = Open3.capture3(command)
